...
VIBE_ROOT = REPO_ROOT / "vibe-trading" / "agent"
...
# ─── Legacy reference (post-merge baseline semantics of _load_trades) ───
#
# This mirrors the FINAL intended column-based implementation: per-column
# extraction with column-missing defaults, plus the merged semantic-fix
# timestamp behaviour. It is intentionally NOT the stale incoming Build 7
# baseline (which raised on invalid timestamps and substituted 2000-01-01 for
# NaN). The authoritative timestamp semantics are:
#   - timestamp COLUMN absent           -> "2000-01-01" sentinel default
#   - timestamp COLUMN present, blank/NaN -> NaT (coerce, no sentinel)
#   - timestamp COLUMN present, invalid   -> NaT (coerce, never raises)
#   - timestamp COLUMN present, valid     -> parsed normally
...
def _legacy_load_trades(run_dir: Path) -> List[TradeRecord]:
    ...
    # Timestamp-present path coerces blank/NaN/invalid values to NaT; only the
    # missing COLUMN falls back to the "2000-01-01" sentinel.
    if "timestamp" in exit_rows.columns:
        timestamps = pd.to_datetime(exit_rows["timestamp"], errors="coerce").tolist()
    else:
        timestamps = [pd.Timestamp("2000-01-01")] * row_count
    quantities = (
...
def _cases() -> List[Callable[[], bool]]:
    ...
    def case_c_nan_blank_timestamps() -> bool:
        # Blank/None cells become NaN on read -> coerce to NaT (column present).
        rows = [
    ...
    def case_d_invalid_timestamps() -> bool:
        # Non-parseable timestamp in an existing column -> coerce to NaT (no raise).
        rows = [
            {"timestamp": "not-a-date", "code": "AAPL", "side": "sell", "qty": 1,
             "price": 100, "pnl": 5, "return_pct": 0.01, "reason": "signal",
             "holding_days": 1},
        ]
        return _run_case("D invalid timestamps coerce to NaT", rows, full_cols)
